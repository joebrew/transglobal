...
\vspace{10mm}
<<fig = TRUE , height = 3.7, results=hide>>=
setwd("C:/Users/BrewJR/Documents/transglobal/protocol")
library(rgdal)


library(devtools)
# For getting shapefiles
source_url("https://raw.githubusercontent.com/joebrew/maps/master/gadm_function.R")
...
gadm(country = "MOZAMBIQUE", level = 0)
gadm(country = "MOZAMBIQUE", level = 1)
#gadm(country = "MOZAMBIQUE", level = 2)
gadm(country = "MOZAMBIQUE", level = 3)
gadm(country ="MALAWI", level = 1)

tanzania1 <-readOGR("spatial/tan", "TAN")
tanzania0 <- collapse_map(tanzania1)

gadm(country ="ZIMBABWE", level = 2)
gadm(country ="ZAMBIA", level = 2)
gadm(country ="SWAZILAND", level = 1)
gadm(country ="SOUTH AFRICA", level = 2)
gadm(country ="BOTSWANA", level = 2)
gadm(country ="NAMIBIA", level = 2)
gadm(country ="DEMOCRATIC REPUBLIC OF THE CONGO", level = 2)
gadm(country ="MADAGASCAR", level = 2)
gadm(country = "ANGOLA", level = 2)

par(oma = c(1,1,1,1))
par(mar = c(1,1,1,1))
my_colors <- colorRampPalette(c("darkorange", "darkred", "brown"))(nrow(mozambique3))
my_colors <- sample(my_colors, length(my_colors))
plot(mozambique3, col = my_colors, border = NA)
plot(mozambique1, add = T, lwd = 0.2)

add_map <- function(object){
  my_colors <- colorRampPalette(c("grey", "black"))(nrow(object))
  my_colors <- sample(my_colors, length(my_colors))
  my_colors <- adjustcolor(my_colors, alpha.f = 0.6)
  
  border <- collapse_map(object)
  
  plot(object, add = T, col = my_colors, border = NA)
  plot(border, add = T)
}

add_map(object = malawi1)
add_map(tanzania1)
plot(tanzania0, add = T)
add_map(malawi1)
add_map(tanzania1)
add_map(zimbabwe2)
add_map(zambia2)
add_map(swaziland1)
add_map(southafrica2)
add_map(botswana2)
add_map(namibia2)
add_map(congothedemocraticrepublicofthe2)
add_map(madagascar2)
add_map(angola2)
